...
const User=require('./../models/Resturent');
const {jwtAuthMiddleware,generateToken}=require('./../jwt')
route.post('/signup',async(req,res)=>{
    try{        
        const data=req.body
        const existingUser = await User.findOne({ Organization_Email: data.Organization_Email });
        if (existingUser) {
...
route.post('/login',async(req,res)=>{
    try{
        ...
        const user=await User.findOne({Organization_Email:Organization_Email})
